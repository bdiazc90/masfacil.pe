...
export function orderOffers(offers, criterion = 'distance') {
  const accessors = {
    distance: (offer) => offer.distance_km,
    price: (offer) => offer.price,
    freshness: (offer) => offer.age_days,
  };
  ...
  return offers.map((offer, index) => ({ offer, index })).sort((left, right) => {
    const difference = accessor(left.offer) - accessor(right.offer);
    return difference || left.offer.id.localeCompare(right.offer.id) || left.index - right.index;
  }).map(({ offer }) => offer);
}
