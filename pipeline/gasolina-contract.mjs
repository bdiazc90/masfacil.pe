...
export const GASOLINA_MANIFEST_VERSION = '2.0.0';
...
const sameKeys = (value, keys) => value && !Array.isArray(value) && JSON.stringify(Object.keys(value)) === JSON.stringify(keys);
...
export function validateGasolinaDataset(dataset) {
  ...
  if (dataset?.schema_version !== GASOLINA_MANIFEST_VERSION || !text(dataset?.revision_id)) errors.push('versión o revisión inválida');
  ...
  for (const offer of dataset?.offers ?? []) {
    if (!sameKeys(offer, PUBLIC_OFFER_FIELDS)) { errors.push('allowlist de oferta inválida'); continue; }
    if (!/^g2_[a-f0-9]{24}$/.test(offer.id) || !Number.isFinite(offer.price) || offer.price <= 0 || !timestamp(offer.reported_at) || !text(offer.district) || !Number.isFinite(offer.longitude) || offer.longitude < -82 || offer.longitude > -68 || !Number.isFinite(offer.latitude) || offer.latitude < -19 || offer.latitude > 1) errors.push('valor de oferta inválido');
  }
  return errors;
}
...
export function validateGasolinaManifest(manifest) {
  ...
  if (manifest?.schema_version !== GASOLINA_MANIFEST_VERSION || !text(manifest?.revision_id)) errors.push('versión o revisión de manifest inválida');
...
export function validateGasolinaRefreshState(state, manifest) {
  ...
  if (state?.schema_version !== GASOLINA_MANIFEST_VERSION || state?.revision_id !== manifest?.revision_id) errors.push('revisión refresh-state inválida');
...
export function validateGasolinaBundle(manifest, key, body) {
  ...
  try {
    ...
    if (dataset.revision_id !== manifest.revision_id || dataset.product.key !== key || dataset.cutoff_at !== descriptor.cutoff_at) errors.push('snapshot no coincide con descriptor');
